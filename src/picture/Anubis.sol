...
contract Anubis {
	constructor() public{
		...
		goIntoPyramid(p2, 0);
		selectCard(p1, 0, false);
		selectCard(p2, 0, false);
		selectCard(p1, 0, true);
		selectCard(p2, 0, true);
		selectCard(p1, 0, false);
		selectCard(p2, 0, false);
		selectCard(p1, 0, false);
		selectCard(p2, 0, false);
	...
	mapping(uint=>uint8[]) public BattleHistory; // {AC = 0, AB = 1, BC = 2, BB = 3}

	uint public pyramidCounter = 0;
